...
class _AddClientScreenState extends State<AddClientScreen> {
  ...
  void initState() {
    ...
    _serviceTypeController = TextEditingController(text: existing?.serviceType ?? '');
    _emailController = TextEditingController(text: existing?.contactEmail ?? '');
    _phoneController = TextEditingController(text: existing?.contactPhone ?? '');
    ...
    _paymentType = _sessionFrequency == 'Avulso' ? PaymentType.avulso : (existing?.paymentType ?? PaymentType.mensal);
    _rateType = existing?.rateType ?? (labels.flatRate ? RateType.total : RateType.perHour);
    ...
    if (existing != null && existing.sessionFrequency == 'Avulso') {
      ...
      _avulsoDateTime = combineDateAndTime(_startDate, const TimeOfDay(hour: 18, minute: 0));
    } else {
  ...
  Future<void> _pickSlotTime(int weekday) async {
    ...
    final picked = await pickTimeWheel(context, current);
  ...
  Future<void> _pickStartDate() async {
    final picked = await showDatePicker(
      ...
      selectableDayPredicate: _selectedWeekdays.isEmpty ? null : (date) => _selectedWeekdays.contains(date.weekday),
    );
    if (picked == null) return;
    setState(() {
  ...
  Future<void> _pickAvulsoDateTime() async {
    ...
    final pickedTime = await pickTimeWheel(context, TimeOfDay.fromDateTime(_avulsoDateTime));
    if (pickedTime == null) return;

    setState(() {
      _avulsoDateTime = DateTime(pickedDate.year, pickedDate.month, pickedDate.day, pickedTime.hour, pickedTime.minute);
      _startDate = dateOnly(_avulsoDateTime);
    });
  }
  ...
  String _weekdayMismatchMessage(WeekdayMismatch mismatch) {
    ...
    return s.weekdayMismatch(_formatDate(_startDate), s.weekdayFull(mismatch.actualWeekday), allowed);
  }
  ...
  void _save() {
    ...
    if (_isAvulso) {
      slots = [WeeklySlot(weekday: _avulsoDateTime.weekday, time: TimeOfDay.fromDateTime(_avulsoDateTime))];
      ...
      if (_selectedWeekdays.length != required) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text(s.selectExactlyDays(required))),
        );
        return;
      }

      slots = _sortedWeekdays.map((wd) => WeeklySlot(weekday: wd, time: _slotTimes[wd]!)).toList();
      ...
      if (startMismatch != null) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text(s.startDateError(_weekdayMismatchMessage(startMismatch)))),
        );
        return;
      }
      if (endDate == null && !_noEndDate) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text(s.indicateEndDate)),
        );
        return;
      }
      if (endDate != null) {
        ...
        if (endMismatch != null) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text(s.endDateError(_weekdayMismatchMessage(endMismatch)))),
          );
          return;
        }
      }
    ...
    final rate = double.parse(_hourlyRateController.text.trim().replaceAll(',', '.'));

    final client = Client(
      id: widget.existingClient?.id ?? DateTime.now().microsecondsSinceEpoch.toString(),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        leadingWidth: 100,
        title: Text(
          _isEditing ? s.editNoun(labels.clientSingular) : s.newLabel(labels.clientSingular),
          style: const TextStyle(fontSize: 22, fontWeight: FontWeight.w700, color: AppColors.neutralDark),
        ),
        centerTitle: true,
        actions: [
          TextButton(
            onPressed: _save,
            child: Text(s.save, style: const TextStyle(fontSize: 16, fontWeight: FontWeight.w600)),
          ),
        ],
      ),
      body: SafeArea(
              ...
              children: [
                ...
                Container(
                  ...
                  child: DropdownButtonFormField<ActivityType>(
                    key: ValueKey('clientActivityType-${_clientActivityType.name}'),
                    ...
                    items: ActivityType.values.map((type) {
                      ...
                      return DropdownMenuItem(
                        ...
                        child: Row(
                          ...
                          children: [
                            Icon(iconForActivityType(type), color: color, size: 18),
                    ...
                    onChanged: (value) {
                      if (value != null) setState(() => _clientActivityType = value);
                    },
                  ),
                ),
                ...
                Container(
                  ...
                  child: Column(
                    children: [
                      SchedifyTextField(controller: _nameController, label: s.nameLabel, validator: _validateName),
                      ...
                      const SizedBox(height: 14),
                      SchedifyTextField(controller: _notesController, label: s.notesLabel, maxLines: 4),
                    ],
                  ),
                ),
                const SizedBox(height: 22),
                FormSectionTitle(title: s.sessionConfigSection(sessionLower)),
                const SizedBox(height: 12),
                Container(
                  padding: const EdgeInsets.all(18),
                  decoration: BoxDecoration(color: AppColors.surfaceWhite, borderRadius: BorderRadius.circular(22)),
                  child: Column(
                    children: [
                      DropdownButtonFormField<int>(
                        key: ValueKey('duration-${_customDuration ? _customDurationSentinel : _sessionDurationMinutes}'),
                        initialValue: _customDuration ? _customDurationSentinel : _sessionDurationMinutes,
                        decoration: schedifyInputDecoration(labels.durationLabel),
                        items: [
                          ..._presetDurations.map((m) => DropdownMenuItem(value: m, child: Text(s.minutesOption(m)))),
                          DropdownMenuItem(value: _customDurationSentinel, child: Text(s.customDurationOption)),
                        ],
                        onChanged: (value) {
                      ...
                      DropdownButtonFormField<String>(
                        ...
                        decoration: schedifyInputDecoration(labels.frequencyLabel),
                        items: kSessionFrequencyKeys
                            .map((f) => DropdownMenuItem(value: f, child: Text(sessionFrequencyLabel(f, widget.language))))
                            .toList(),
                        onChanged: (value) {
                      ...
                      if (!_isAvulso) ...[
                        ...
                        WeekdaySelector(
                          ...
                          weekdayShortLabels: List.generate(7, (i) => s.weekdayShort(i + 1)),
                        ),
                        const SizedBox(height: 14),
                        ..._sortedWeekdays.map((wd) {
                          final time = _slotTimes[wd] ?? const TimeOfDay(hour: 18, minute: 0);
                          return Padding(
                            ...
                            child: InkWell(
                              ...
                              child: InputDecorator(
                                decoration: schedifyInputDecoration(s.hourFor(s.weekdayShort(wd))),
                                child: Row(
                                  children: [
                                    Expanded(
                                      child: Text(
                                    ...
                                    const Icon(Icons.access_time, size: 18, color: AppColors.neutralSoft),
                                  ],
                                ),
                              ),
                            ),
                        ...
                        Row(
                          children: [
                            Expanded(
                              child: Text(s.noEndDateLabel, style: const TextStyle(fontSize: 15, color: AppColors.neutralMedium)),
                            ),
                            Switch(
                              value: _noEndDate,
                              onChanged: (value) => setState(() => _noEndDate = value),
                  ...
                  child: Column(
                    children: [
                        ...
                        children: [
                          Expanded(
                            child: SchedifyTextField(
                              ...
                              label: _rateType == RateType.total ? s.rateLabelTotal : s.rateLabelPerHour,
                          ...
                          SegmentedButton<RateType>(
                            segments: [
                              ButtonSegment(value: RateType.perHour, label: Text(s.perHourOption)),
                              ButtonSegment(value: RateType.total, label: Text(s.totalOption)),
                            ],
                            selected: {_rateType},
                            showSelectedIcon: false,
                            onSelectionChanged: (selection) => setState(() => _rateType = selection.first),
                          ),
                        ],
                      ),
                      const SizedBox(height: 14),
                      DropdownButtonFormField<PaymentType>(
                        ...
                        decoration: schedifyInputDecoration(s.paymentFrequencyLabel),
                        items: PaymentType.values
                            .map((t) => DropdownMenuItem(value: t, child: Text(paymentTypeLabel(t, widget.language))))
                            .toList(),
                        onChanged: _isAvulso
                            ? null
                            : (value) {
                ...
                Container(
                  ...
                  decoration: BoxDecoration(color: AppColors.brandBlueSoft, borderRadius: BorderRadius.circular(18)),
                  child: Text(
                    s.scheduleHint(labels.sessionPlural.toLowerCase()),
                    style: const TextStyle(fontSize: 14, color: AppColors.neutralMedium, height: 1.4),
                  ),
                ),
              ],
            ),
